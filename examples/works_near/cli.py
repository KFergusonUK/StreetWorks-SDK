...
Queries the v1 subset documented on examples.works_near.query: Traffic
Wales always (keyless); National Highways if --nh-key / NH_SUBSCRIPTION_KEY
is set; Street Manager if SM_EMAIL / SM_PASSWORD are set *and* --usrn is
...
cover every registered provider - see examples.works_near.query's own
module docstring for why this whole thing lives under examples/, not as
a streetworks package export.
...
Run: python -m examples.works_near.cli
     python -m examples.works_near.cli --lat 51.48 --lon -3.18 --radius-m 10000
     python -m examples.works_near.cli --usrn 33909869   # needs SM_* in the env
"""
...
from .query import v1_providers, works_near
