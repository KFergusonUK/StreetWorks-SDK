...
**Why this lives under ``examples/``, not ``src/streetworks/``.** An
earlier version of this module shipped inside the installable package,
with ``works_near``/``works_near_usrn``/``NearbyWorks`` exported from
``streetworks``'s own top-level ``__all__`` - right next to
``providers()``/``get_provider()``, which genuinely do span every
registered provider. That's a real expectation mismatch: a caller doing
``from streetworks import works_near`` has no signal at the call site
that this is a curated four-provider UK-only subset, not a query over
this SDK's full coverage (Denmark, Switzerland, Austria, ...) - a point
outside the UK would just silently return an empty list, the same
quiet-failure-mode this SDK's own "grade honestly" discipline argues
against everywhere else. Keeping this as example code rather than a
package export makes the UK-only scope obvious from where the code
lives, not just from a docstring caveat a caller may never read.
...
* Credentials-wanted / unverified / :class:`~streetworks.exceptions.ProviderUnavailableError`
  scaffolds (Trafikverket, Vejdirektoratet, SA, Stockholm, MapRoad,
  Greece, Austria/ASFINAG, LINZ Roads) - never queried, even if a caller
  later grows this module's allowlist by mistake: :func:`v1_providers`
  consults ``ProviderEntry.verified`` and ``credentials``.
...
from streetworks.registry import _REGISTRY, ProviderEntry

if TYPE_CHECKING:
    from streetworks.common.models import Coordinate, Works
    from streetworks.datex2 import NationalHighwaysClient
...
def _fetch_traffic_wales() -> list[Works]:
    from streetworks.common import from_trafficwales
