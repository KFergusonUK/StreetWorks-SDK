...
def test_credentials_wanted_is_the_only_unverified_tier():
    ...
    Nova Scotia 511 (`ns511`) and 511 Yukon (`yt511`) are all better-off
    than every other entry in this tier: each needs a real developer key
    this build doesn't have, but the field schema itself isn't a guess or
    ...
    matching field-for-field, and every one of the six answers the
