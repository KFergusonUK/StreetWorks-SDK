...
def test_credentials_wanted_is_the_only_unverified_tier():
    ...
    511 Yukon (`yt511`), Georgia 511 (`ga511`), Louisiana 511 (`la511`)
    and Connecticut 511 (`ct511`) are all better-off than every other
    entry in this tier: each needs a real developer key this
    ...
    docstring. 511 Alberta (`ab511`), Nevada 511 (`nv511`) and Alaska
    511 (`ak511`) were in this same tier until a real developer key
    confirmed exactly that live for each (2026-08-22) - they're verified
    now, the first three key-gated jurisdictions on this platform to
    graduate out of this tier.
