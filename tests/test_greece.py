"""Tests for the Greece documented scaffold.

This is not a functional adapter - see the module docstring in
``streetworks.greece.client``. No fixture exists because there is
nothing to parse: ``GreeceClient`` always raises
...
from streetworks.greece import GreeceClient
...
def test_raise_happens_regardless_of_arguments_passed():
    ...
    a genuinely absent roadworks source, not a missing key."""
    with pytest.raises(ProviderUnavailableError):
        GreeceClient(api_key="anything", base_url="https://example.test")
...
def test_no_network_call_is_made():
    ...
    source = inspect.getsource(GreeceClient)
    assert "httpx" not in source
    assert "requests" not in source


# --------------------------------------------------------------------------- #
# Registry consistency - greece is registered honestly, not silently omitted.
# --------------------------------------------------------------------------- #


def test_greece_is_registered_and_unverified():
    entry = _get_entry("greece")
    assert entry.verified is False
    assert entry.client is GreeceClient


def test_greece_client_construction_raises_even_via_the_registry():
    entry = _get_entry("greece")
    with pytest.raises(ProviderUnavailableError):
        entry.client()


def test_get_provider_resolves_greece_but_raises_on_use():
    GreeceClientResolved = get_provider("greece")
    assert GreeceClientResolved is GreeceClient
    with pytest.raises(ProviderUnavailableError):
        GreeceClientResolved()
