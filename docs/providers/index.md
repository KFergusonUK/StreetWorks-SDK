...
**Europe**  ✓ Austria · Belgium · Bulgaria · Denmark · Finland · France · Germany · Gibraltar · Guernsey · Iceland · Italy · Jersey · Lithuania · Luxembourg · Netherlands · Norway · Portugal · Spain · Switzerland · Greece · Ireland · UK   ~ Sweden
...
distinguishes these. **Canada, Portugal, Austria, Ireland, and Greece
all moved from in-progress to live** once DriveBC (British Columbia),
Lisboa (Condicionamentos de Trânsito), Vienna (verkehrswirksame
Baustellen), Monaghan's own road network, and Marousi's own street
register shipped — real coverage in each is still partial (one
province/city/county/municipality, not national — Austria's own
national ASFINAG feed stays a Credentials-wanted scaffold, Ireland's
own national MapRoad roadworks stays a documented-unavailable scaffold,
and Greece's own national roadworks stays documented-unavailable too),
same as every other ✓ entry that isn't literally comprehensive; see
[`docs/providers/canada.md`](canada.md),
[`docs/providers/portugal.md`](portugal.md),
[`docs/providers/austria.md`](austria.md),
[`docs/providers/ireland.md`](ireland.md) and
[`docs/providers/greece.md`](greece.md) for exactly what's covered
...
| `streetworks.marousi` | [Δήμος Αμαρουσίου (Marousi)](https://gis.maroussi.gr/geoserver/wfs) — a real Greek municipality's own street-name register (no credentials, confirmed live 2026-08-17, 721 real features, 100% named), this SDK's first Greek gazetteer coverage. A pilot for a real per-municipality fan-out (580 real datasets on `data.gov.gr`), not a national build — Greece's official INSPIRE geoportal times out completely, the same real failure `streetworks.greece` already documented for `nap.gov.gr`. No stated point/line field on this minimal schema — every `Street` carries `GeometryGrade.ABSENT`, the real polygon preserved in `.raw` only. **Licence genuinely unstated** — every one of the 580 real municipal datasets checked shows "License not specified"; built on instruction. See [`docs/providers/greece.md`](greece.md#δήμος-αμαρουσίου-marousi) | read |
| `streetworks.swisstopo` | [Amtliches Verzeichnis der Strassen](https://data.geo.admin.ch/api/stac/v0.9/collections/ch.swisstopo.amtliches-strassenverzeichnis) — Switzerland's federal street-name register, fed daily from the Gebäude- und Wohnungsregister (GWR) and republished by swisstopo (no credentials, confirmed live 2026-08-18, 224,985 real records, 100% carrying a real name and coordinate, zero duplicate IDs). This SDK's first Swiss streets/gazetteer provider. Bulk CSV chosen over a real, live, keyless point-query API since that API only supports search, not "everything" — the same trade-off ANNCSU (Italy) made. Geometry is a real single point only (`EPSG:2056`, Swiss LV95, never swapped) — the live API's own richer `LineString` and the bulk File Geodatabase/XTF formats exist too, but the XTF alone is a real 842 MB file, too large for this SDK's no-heavy-GIS-dependency convention. A real Liechtenstein-scoped sibling resource exists on the same collection, not fetched here. Licence: swisstopo's own OGD terms (free, commercial use permitted, attribution required). See [`docs/providers/switzerland.md`](switzerland.md#amtliches-verzeichnis-der-strassen-swisstopo) | read |
| `streetworks.vlaanderen` | [Straatnamenregister](https://api.basisregisters.vlaanderen.be/v2/straatnamen) — Flanders' own street-name register, part of Basisregisters Vlaanderen (no credentials, confirmed live 2026-08-20, roughly 99,600 real street names). This SDK's first Belgian streets/gazetteer provider, Flanders-only like the existing Belgian roadworks coverage. Not the layer first checked — Informatie Vlaanderen's own Wegenregister WFS has real line geometry but a genuinely two-sided street-name shape (`linkerstraatnaam`/`rechterstraatnaam` can differ) closer to NWB's segment-aggregation model; the dedicated `Straatnaam` REST resource was used instead. No geometry on this resource — `GeometryGrade.ABSENT` on every `Street`, the same shape ANNCSU/BEV already established. A real, confirmed API quirk: the documented `gemeenteniscode` municipality filter is silently ignored; an undocumented `gemeentenaam=` text filter works but would need an unattempted ~300-municipality fan-out, so `administrative_area` is left unresolved. Licence: Flanders' standard free-reuse terms (stated default, not per-dataset-confirmed). See [`docs/providers/europe.md`](europe.md#straatnamenregister-flanders-belgium) | read |
| `streetworks.registrucentras` | [Adresų registras](https://get.data.gov.lt/datasets/gov/rc/ar/gragatve/GraGatve) — Lithuania's national street-centerline register, run by Registrų centras (no credentials, confirmed live 2026-08-20, 22,547 real records, 100% named with real geometry). This SDK's first Lithuanian streets/gazetteer provider. The dataset's own promoted download link bakes a version number into the URL; a shorter, undated link on the same page redirects to a real, stable, version-less route, used instead. A real axis-order quirk — the same one this SDK's own Via Lietuva roadworks provider already documented — confirmed independently: `gatves` WKT states coordinates as `(Northing, Easting)`, not the standard `(X, Y)` order. Reprojected client-side from LKS-94 (`EPSG:3346`) via a new closed-form transform. `administrative_area` left unresolved — the real settlement reference would need a disproportionate separate 127 MB dataset to label one field. Licence CC BY 4.0. See [`docs/providers/europe.md`](europe.md#adresų-registras-registrų-centras-lithuania-streets) | read |
| `streetworks.hamburg` | [Zentraler AdressService Hamburg (GAGES)](https://qs-api.hamburg.de/datasets/v1/gages_vereinfacht/collections/strassen/items) — Hamburg's own street gazetteer, run jointly by the state statistics and surveying offices (no credentials, confirmed live 2026-08-20, 9,639 real records, 100% named). This SDK's first German state-level streets/gazetteer provider — Berlin was checked first but its entire GDI WFS host is confirmed live to be down for maintenance with no ETA, a real blocker not routed around. Real Point geometry, genuinely reprojected server-side to WGS84 by default. Real, standard OGC API Features pagination (`links` with `rel: "next"`). `administrative_area` is a per-provider constant (`"Hamburg"`) — a finer real Ortsteil code exists inline but no lookup collection resolves it to a name. Licence: Datenlizenz Deutschland - Namensnennung - 2.0. See [`docs/providers/europe.md`](europe.md#zentraler-adressservice-hamburg-gages) | read |
| `streetworks.brandenburg` | [WFS BB-BE Gazetteer](https://isk.geobasis-bb.de/ows/gazetteer_wfs) — Brandenburg's own street gazetteer, run by the state surveying and geoinformation agency (no credentials, confirmed live 2026-08-20, 52,902 real records). This SDK's second German state-level streets/gazetteer provider. A real, confirmed GML-only WFS — no JSON output format exists, so this module parses real GML/XML directly via `xml.etree.ElementTree` rather than the shared JSON-first OGC client. `administrative_area` is reconstructed from two real fields (`ortsnamePost`/`zusatzOrtsname`), confirmed to match the record's own `gemeindename_normalisiert`. No geometry on this resource — the only real geometry stated is a `Polygon` (areal extent), so `GeometryGrade.ABSENT` on every `Street`, the same shape `from_marousi_street` already established. A real, live-confirmed, non-exhaustive Berlin presence (8/500 real sample rows carry Berlin's own state code) — scoped and documented as Brandenburg's own provider, not exhaustive Berlin coverage. Licence: Datenlizenz Deutschland - Namensnennung - 2.0. See [`docs/providers/europe.md`](europe.md#wfs-bb-be-gazetteer-brandenburg) | read |
| `streetworks.geosn` | [GeoSN Hauskoordinaten](https://geodaten.sachsen.de/downloadbereich-hauskoordinaten-4172.html) — Saxony's statewide address-point export, deduplicated to streets (no credentials, confirmed live 2026-08-20, 990,090 real address rows deduplicated to 42,824 real distinct streets). This SDK's third German state-level streets/gazetteer provider, completing the "state fan-out" (Hamburg, Brandenburg, Saxony, Berlin — Berlin's own GDI host remains blocked). Not the shared Deutschland-Online-Gazetteer WFS Hamburg/Brandenburg use — confirmed Saxony doesn't participate in it, so this uses a real statewide bulk CSV export instead (the largest single download in this SDK's German-state cluster, ~51 MB zipped). Geometry is a real address point, reprojected client-side from ETRS89/UTM zone 33N (`EPSG:25833`, no `pyproj`). `administrative_area` carries the real `gmd` municipality name directly. Licence: Datenlizenz Deutschland - Namensnennung - 2.0. See [`docs/providers/europe.md`](europe.md#geosn-hauskoordinaten-saxony) | read |
| `streetworks.caclr` | [CACLR](https://data.public.lu/en/datasets/registre-national-des-localites-et-des-rues/) — Luxembourg's national street register, run by the Administration du Cadastre et de la Topographie (no credentials, confirmed live 2026-08-20, 9,946 real records, 100% named). This SDK's first Luxembourgish streets/gazetteer provider. A real fixed-width legacy export (field layout confirmed from ACT's own published import script), joined via a real composite `(canton, commune)` key after a naive single-key join was found live to resolve a real Luxembourg-City street to the wrong commune. The client resolves the current download URL from data.public.lu's own stable udata API each call, rather than hardcoding a dated snapshot link. No geometry on this resource — `GeometryGrade.ABSENT` on every `Street`, the same shape ANNCSU/BEV already established. Licence: CC0 — the most permissive of any provider in this SDK. See [`docs/providers/europe.md`](europe.md#caclr-registre-national-des-localités-et-des-rues-luxembourg) | read |
| `streetworks.bev` | [Österreichisches Adressregister (BEV)](https://data.bev.gv.at/download/Adressregister/Archiv_Adressregister/) — Austria's national street-name register, run by the Federal Office of Metrology and Surveying (no credentials, confirmed live 2026-08-18, 137,767 real records, 100% carrying a real name). This SDK's first Austrian streets/gazetteer provider — not the paid per-record shop product first found on `bev.gv.at`, but a separate free CC-BY-4.0 line on `data.bev.gv.at`. Joined against a real municipality table so `administrative_area` carries a resolved name, not a bare code. No geometry anywhere in this resource — `GeometryGrade.ABSENT` on every `Street`, the same shape ANNCSU (Italy) already established; real coordinates exist only on a much larger sibling address-level resource, deliberately not fetched. A real, disclosed limitation: a periodically dated snapshot URL with no stable "latest" alias found. See [`docs/providers/austria.md`](austria.md#österreichisches-adressregister-bev) | read |
| `streetworks.dar` | [Danmarks Adresseregister (DAR)](https://services.datafordeler.dk/DAR/DAR/3.0.0/rest) — Denmark's national named-road register on Datafordeleren (no credentials, confirmed live 2026-08-18, 99.96% of a 5000-record sample carries a real name). This SDK's first Danish streets/gazetteer provider — not the originally checked DAWA (`api.dataforsyningen.dk`), whose own docs carry a live "DAWA lukker" (closing) warning, confirmed via web search to be phased out toward 1 October 2026. Only CRS stated is ETRS89/UTM32N (`EPSG:25832`) — a `srid` reprojection parameter was tried and rejected (real `400`); reprojected client-side via a new closed-form Transverse Mercator inverse, cross-checked against DAWA's own real WGS84 output for the same road. A real three-tier geometry fallback: line, then a real connection-point field, then genuinely `GeometryGrade.ABSENT` — never a polygon ring forced into `Coordinate`. Licence CC BY 4.0. See [`docs/providers/denmark.md`](denmark.md#danmarks-adresseregister-dar) | read |
...
| `streetworks.greece` | Greece — registered as a documented, honestly-unavailable scaffold, the same tier as MapRoad. Its real NAP ([nap.gov.gr](https://data.nap.gov.gr/)) carries only POI/sensor data (truck parking, VMS/VDS, weather, floating car data) — no roadworks or DATEX II Situation dataset at all, confirmed via its own real dataset titles. The portal is also currently unreachable (a real live 502). `GreeceClient()` always raises `ProviderUnavailableError` rather than guessing. See [`docs/providers/greece.md`](greece.md#greece-documented-unavailable) | read |
...
| `streetworks.amsterdam` | [WIOR](https://api.data.amsterdam.nl/v1/wior/wior/) — Gemeente Amsterdam's public-space-works coordination register (no credentials, confirmed live 2026-08-18, 10,063 real records), this SDK's first Dutch municipal roadworks provider, a sibling to the existing national coverage (NDW, NWB, BAG) at city scale. Geometry is real Polygon/MultiPolygon only — the first ring's first vertex is used as a representative point, the same discipline `from_oslo`/`from_canton_zurich` apply; unlike Denmark's DAR, this endpoint genuinely honours server-side reprojection to WGS84 via a real `Accept-Crs` header. A real data-quality quirk kept rather than normalised away: one live record carries `hoofdstatus='Yes'` instead of a genuine Dutch status value. Licence: Gemeente Amsterdam's own open-data terms (free reuse, not asserted as CC0). See [`docs/providers/europe.md`](europe.md#amsterdam-wior) | read |
...
wanted](#credentials-wanted), below, plus MapRoad
Roadworks Licensing and Greece (neither has a usable interface a credential
...
**Two further, genuinely different cases: MapRoad Roadworks Licensing
and Greece aren't access-blocked in the usual sense.** Kept in a
separate table rather than folded into the one above, since
"Credential"/"How to get it" don't quite apply the way they do for the
other four. Road Report NT used to sit here as a documented-unavailable
scaffold; it graduated on 2026-08-19 after `GET /api/Obstruction/GetAll`
was confirmed live — see [Recently confirmed](#recently-confirmed) and
[`docs/providers/australia.md`](australia.md).
...
- **Road Report NT (Northern Territory)** — 2026-08-19. The
  documented-unavailable scaffold graduated after a real, credential-free
  pull of `GET /api/Obstruction/GetAll`: HTTP 200, 140 CURRENT records,
  26 official `Roadworks` (type-code 28). `iter_roadworks()` returns
  only that works slice; the reverse-engineered SignalR hub is not
  consumed. See [`docs/providers/australia.md`](australia.md).
