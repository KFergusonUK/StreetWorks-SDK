...
| `streetworks.osni` | [OSNI Open Data - Gazetteer - Streetnames](https://www.opendatani.gov.uk/) — Northern Ireland's street-name gazetteer (no credentials, confirmed live 2026-08-16, 25,643 real features). Jurisdiction-distinct, never folded under UK. The documented ArcGIS REST MapServer endpoint is currently down (the whole `services.spatialni.gov.uk` domain redirects to a broken holding page), so this client uses the real bulk-download route instead. Uses the feed's own `X_Coord`/`Y_Coord` (Irish Grid, `EPSG:29903` inferred from coordinate plausibility since the endpoint that would confirm it live is down) rather than the download's own reprojected WGS84 geometry. Carries a real, fully-unique `USRN` field, scoped to `OSNI` rather than presented as a GB-national identifier. Graded honestly as a name+point gazetteer — no ASD-style richness. Licence OGL v3.0. See [`docs/providers/uk.md`](uk.md#osni-streetnames-northern-ireland-gazetteer) | read |
