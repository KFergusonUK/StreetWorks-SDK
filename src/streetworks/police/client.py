...
inclusion here. This is an initial, thin adapter over the crime and
force/neighbourhood-lookup endpoints; stop-and-search and the fuller
neighbourhood catalogue (team/events/priorities/boundary) aren't wrapped yet
...
class PoliceClient:
    ...
    def street_level_crimes_in_area(
        ...
        *,
        ...
        """``GET /crimes-street/{category}`` for a custom polygon - a list of
        ``(lat, lng)`` points describing a worksite boundary or route, rather
        than a single point with an implicit radius."""
        poly = ":".join(f"{lat},{lng}" for lat, lng in points)
        return self._get(f"crimes-street/{category}", poly=poly, date=date)
